...
module control_unit (
  ...
  always_comb begin
    ...
    if (execute_config_o) cycle_count_load_config_d = '0;
    else if (!data_config_done_i) cycle_count_load_config_d = cycle_count_load_config_q + 1;

    if (execute_output_o) cycle_count_execute_d = '0;
    else if (!data_output_done_i) cycle_count_execute_d = cycle_count_execute_q + 1;

    if (execute_output_o) cycle_count_stall_d = '0;
    else if (data_read_stall_i || data_write_stall_i) cycle_count_stall_d = cycle_count_stall_q + 1;
